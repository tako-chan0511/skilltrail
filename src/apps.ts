...
export interface App {
  ...
  features: string[];
  stack: StackCategory[]; // ★ stackの型を StackCategory[] に変更
}

// ★★★ 全29件のアプリデータを新しい構造に更新 ★★★
export const apps: App[] = [
  ...
  },
  {
    ...
    features: [ 'Supabase Authによるメール・パスワード認証', 'RLSによるユーザーデータの厳格なアクセス制御', 'キーワードと複数タグによるAND/OR検索機能', 'Service Workerを利用したPWA対応（オフラインキャッシュ）', '認証不要で試せるサンドボックスモード', ],
    stack: [
  ...
  },
  {
    ...
    features: [ '都市名・住所・地図クリックによる多彩な地点指定', 'Leaflet.jsによるインタラクティブ地図表示', 'OpenWeatherMap APIによる詳細な気象データ取得', 'tone.jsによる気象サウンドの再生', 'PWA対応によるネイティブアプリのような体験', ],
    stack: [
        { category: 'アーキテクチャ', technologies: [ { name: 'PWA', color: 'bg-amber-300 text-amber-900' } ] },
  ...
  },
  {
    ...
    features: [ 'バックトラッキング法による問題生成とソルバー機能', '解法テクニックをステップ・バイ・ステップで学べるトレーニングモード', '候補数字のメモ機能や、矛盾箇所のハイライトなどの盤面サポート', 'localStorageを利用した盤面の状態保存・復元機能', 'PWA対応によるオフラインでのプレイ', ],
    stack: [
        { category: 'アーキテクチャ', technologies: [ { name: 'PWA', color: 'bg-amber-300 text-amber-900' } ] },
        { category: 'フロントエンド', technologies: [ { name: 'JavaScript (ES6+)', color: 'bg-yellow-200 text-yellow-800' }, { name: 'HTML5', color: 'bg-orange-200 text-orange-800' }, { name: 'CSS3', color: 'bg-blue-200 text-blue-800' } ] },
  ...
  },
  {
    ...
    features: [ '行と列を個別に指定できるN×M盤面生成機能', '解けることが保証されたシャッフルアルゴリズム', '空きマスと同じ行・列のタイルを一括で移動させる便利機能', 'PWA対応によるオフラインでのプレイ', ],
    stack: [
        { category: 'アーキテクチャ', technologies: [ { name: 'PWA', color: 'bg-amber-300 text-amber-900' } ] },
        { category: 'フロントエンド', technologies: [ { name: 'JavaScript (ES6+)', color: 'bg-yellow-200 text-yellow-800' }, { name: 'HTML5', color: 'bg-orange-200 text-orange-800' }, { name: 'CSS3', color: 'bg-blue-200 text-blue-800' } ] },
        { category: 'コアロジック', technologies: [ { name: 'Algorithm Design', color: 'bg-purple-200 text-purple-800' } ] }
  ...
  },
  {
    ...
    features: [ '行と列を個別に指定できるN×M盤面生成機能', 'Fisher-Yatesアルゴリズムによるカードのシャッフル', '一定時間すべてのカードをプレビューできる「チラ見」機能', 'PWA対応によるオフラインでのプレイ', ],
    stack: [
        { category: 'アーキテクチャ', technologies: [ { name: 'PWA', color: 'bg-amber-300 text-amber-900' } ] },
        { category: 'フロントエンド', technologies: [ { name: 'JavaScript (ES6+)', color: 'bg-yellow-200 text-yellow-800' }, { name: 'HTML5', color: 'bg-orange-200 text-orange-800' }, { name: 'CSS3', color: 'bg-blue-200 text-blue-800' } ] },
